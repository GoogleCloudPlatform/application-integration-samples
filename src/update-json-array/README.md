...
```
2) Navigate to the `update-json-array` directory
```sh
cd src/update-json-array
```
...
If you are creating a new integration:
    i) Enter a name `update-json-array` and description in the Create Integration dialog.
...
7) In the file browser dialog, select `update-json-array.json`, and then click Open. A new version of the integration is created using the uploaded file.
